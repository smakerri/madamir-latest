...
const columnCount = 8;
var successCount = 0;
var rowSuccess= [false, false, false, false, false, false, false, false];
var credits = [];
var targetLine = [];
var creditsPosition = [];
var successBalPosition = [undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined];
var creditItem = 0;
...
constructor(props) {
    ...
    startBallLeft = (3 * (global.widthImage)) - (global.widthBall/2) + global.diff+1;
...
getInitialState = () => ({
  ...
  matrix: levels[indexLevel],
   ...
   positions: [[empty_image,empty_image,left_image,right_image,right_image,left_image,empty_image,empty_image],
    ...
    credits : [],
    creditsPosition : [],
    //rowSuccess: [false, false, false, false, false, false, false, false],
...
showImage = (i,j) => {
  ...
  return (<View style={styles.square} key={(i+1) * (j+9)}
                                                              onLayout={event => { const layout = event.nativeEvent.layout; global.opacityWidth = layout.width;global.opacityHeight = layout.height;
                                                              }}>
...
Popup.show({
   ...
   buttonText: 'Rejouer!',
   callback: () => {Popup.hide();indexLevel = 0;stage = 1;
                    this.initGame()}
  })
}
...
Popup.show({
   ...
   buttonText: 'Rejouer',
   callback: () => {Popup.hide(); indexLevel = 0; stage=1; this.initGame();;
   }
  })
} else {
Popup.show({
   ...
   buttonText: 'Suivant',
   callback: () => {Popup.hide(); indexLevel++; stage++; this.initGame();;
   }
  })
  }
}
...
//				<Animated.Image style={[styles.credit_ball_style, this.moveAnimation.getLayout()]} source={ball_image} onLayout={event => {
  //                   const layout = event.nativeEvent.layout;
    ...
    return (
         <Root>
        <View style={styles.container_global_style} onLayout={event => { const layout = event.nativeEvent.layout;
                ...
                <View style={styles.matrix_line_target_style}>
                     {targetLine}
                 </View>
